...
class SpliceFeed(BaseFeed):
    ...
    def __init__(self, googleReader, title):
        ...
        super(SpliceFeed, self).__init__(
            googleReader,
            title      = title,
            id         = GoogleReader.ITEMS_IDS_URL,
            unread     = 0,
            categories = [],
        )
        
        self.fetchUrl = GoogleReader.ITEMS_IDS_URL
...
class GoogleReader(object):
    ...
    ITEMS_IDS_URL       = API_URL + "stream/items/ids"
    ITEMS_CONTENTS_URL  = API_URL + "stream/items/contents"
    ...
    def __str__(self):
        return "<Google reader object: %s>" % self.userId

    def __init__(self, auth, userId=None, userLang="zh-CN"):
        self.auth = auth
        self.feeds = []
        self.categories = []
        self.feedsById = {}
    ...
    def _getFeedContent(self, url, excludeRead=False, continuation=None, number=None, overTime=None):
        """
    ...
    def getSpliceContent(self, excludeRead=False, continuation=None, number=None, overTime=None):
        """
        A list of items (from a feed, a category or from URLs made with SPECIAL_ITEMS_URL)
        ...
        parameters = dict()
        parameters['xt'] = list()
        parameters['xt'].append('user/%s/state/com.google/dislike' % self.userId)
        # if excludeRead:
        #     parameters['xt'].append('user/%s/state/com.google/read' % self.userId)
        if continuation:
            parameters['c'] = continuation
        if number:
            parameters['n'] = number
        ...
        parameters['r'] = 'a'
        parameters['s'] = "splice/user/%s/state/com.google/itemrecs/zh_CN|pop/topic/top/language/zh-CN" % self.userId
        parameters['merge'] = 'true'
        parameters['output'] = 'json'
#        parameters['T'] = self.token
        
        if overTime:
            parameters['ot'] = overTime

        contentJson = self.httpGet(GoogleReader.ITEMS_IDS_URL, parameters)

        result = json.loads(contentJson, strict=False)
        ...
        for item in itemRefs:
            id = item.get('id', None)
            timestampUsec = item.get('timestampUsec', None)
            if id and timestampUsec:
                ids.append(id)
                indexes.append('0')
                it.append(timestampUsec)
        ...
        postParameters = {
            ...
            # 'it' : it,
            'rs' : parameters['s'],
            's' : "pop/topic/top/language/zh-CN"
        }
        # return postParameters
        urlParameters = {'freshness':'false'}
        
        content_url = GoogleReader.ITEMS_CONTENTS_URL
        contentJson = self.httpPost(content_url, postParameters, urlParameters)
        return contentJson
        result = json.loads(contentJson, strict=False)
        return result['items']
    ...
    def getFeedContent(self, feed, excludeRead=False, continuation=None, number=None, overTime=None):
        """
        Return items for a particular feed
        """
        return self._getFeedContent(feed.fetchUrl, excludeRead, continuation, number, overTime)
        
    def getCategoryContent(self, category, excludeRead=False, continuation=None, number=None, overTime=None):
        """
        ...
        return self._getFeedContent(category.fetchUrl, excludeRead, continuation, number, overTime)
    ...
    def httpPost(self, url, post_parameters=None, url_parameters=None):
        ...
        """
        return self.auth.post(url, post_parameters, url_parameters)
...
class AuthenticationMethod(object):
    ...
    """
    def __init__(self):
        self.client = "dogear" #@todo: is this needed?

    def getParameters(self, extraargs=None):
        #ck is a timecode to help google with caching
        parameters = {'client':self.client} #, 'ck': "%s000" % int(time.time())}
        if extraargs:
